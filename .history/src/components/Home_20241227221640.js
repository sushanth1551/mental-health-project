...
const Home = () => {
  ...
  const [isPrivate, setIsPrivate] = useState(false); // State for private/public toggle
  ...
  useEffect(() => {
    ...
    if (storedName) {
      ...
      setShowModal(true); // Show modal if name is not set
    }
  }, []);
  ...
  const handleNameSubmit = () => {
    if (chatbotName.trim() !== "") {
      localStorage.setItem("chatbotName", chatbotName); // Save name to localStorage
      setShowModal(false);
    } else {
      alert("Please enter a valid name!");
    }
  ...
  const handleTogglePrivacy = () => {
    setIsPrivate((prev) => !prev); // Toggle privacy state
  };

  const handleLogout = () => {
    localStorage.removeItem("chatbotName"); // Clear stored name on logout
    setChatbotName("");
  ...
  return (
    <>
      ...
      {/* Add remaining components (e.g., cards, footer) */}
      <Footer />
    </>
  );
};
