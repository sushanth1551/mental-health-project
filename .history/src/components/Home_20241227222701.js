...
import { FaSmile, FaRobot, FaHandshake } from "react-icons/fa";
import { motion } from "framer-motion";
import { Link, useNavigate } from "react-router-dom";
import Nav from "./Nav";
...
const Home = () => {
  ...
  const handleNameSubmit = (e) => {
    ...
    if (chatbotName.trim() !== "") {
      setNameSubmitted(true); // Set name as submitted
      localStorage.setItem("chatbotName", chatbotName); // Save the name in localStorage
    }
  };

  const handleLogout = () => {
    localStorage.removeItem("chatbotName"); // Clear the name from localStorage
    setChatbotName("");
            ...
            <a href="/tasksDone" className="item" style={{ "--position": 4 }}>
              <div className="card">
                ...
                <div className="card-content">
                  <h3>Todo-Tracker</h3>
                  <p>
                    Organize your tasks and stay on top of your to-do list.
                  </p>
                </div>
              </div>
            </a>
